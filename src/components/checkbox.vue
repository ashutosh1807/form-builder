<template>
   <div class="chk draggable" v-if="vis" style="position:absolute">
      ...
      <span class="innerchk" >
         <div class="form-group" :style="Style" >
            <template v-for="(item) in items">
               <label class="custom-control custom-checkbox " :class=" {'custom-control-inline':inline.length >= 1}">
               <input :value="item.value" :name="name" type="checkbox"  class="custom-control-input" > 
               <span class="custom-control-label">{{item.label}} </span>
               </label>
            </template>
         </div>
      </span>
      <b-modal title="Edit Check box"  header-bg-variant="success"  header-text-variant="light"  ref="inp" >
         <div class="modal-content">
            <div class="modal-body">
               ...
               <div align="center">
                  <button class="btn btn-outline-success" @click="items.push({label:'New Option',value:''})">Add new</button>
                  <button class="btn btn-outline-danger" @click="items.pop()">Remove</button>
               </div>
            </div>
         </div>
      </b-modal>
